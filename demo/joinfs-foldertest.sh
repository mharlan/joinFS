...
sqlite3 joinfs.db 'UPDATE OR ROLLBACK directories SET query="SELECT f.inode, f.filename, f.datapath FROM files AS f, metadata AS m, keys AS k WHERE f.inode=m.inode and m.keyid=k.keyid and k.keytext=%s and m.keyvalue=%s;", is_subquery=1, sub_key="test_name" WHERE dirpath="/dirtest2/.jfs_sub_query";'
